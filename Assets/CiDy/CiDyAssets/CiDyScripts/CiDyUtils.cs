...
public static class CiDyUtils
{
    ...
    public static bool BoundsIntersect(Vector3[] bounds, GameObject[] buildings)
    {
        ...
        for (int i = 0; i < buildings.Length; i++)
        {
            ...
            //Prefab Bounds
            Bounds prefabBounds = buildings[i].GetComponentInChildren<Renderer>().bounds;
    ...
    public static Mesh CreateMeshFromPoly(Vector3[] polygon, int resolution, ref VoxelSquare[,] squares)
    {
        ...
        //Convert Original Polygon into Voxels for later use.
        Voxel[] origPoly = new Voxel[polygon.Length];
        ...
        for (int i = 0; i < polygon.Length; i++)
        {
            ...
            //polygon[i] = flatPos;
            origPoly[i] = new Voxel(flatPos, 0.5f);
            origPoly[i].polyIndex = i;
        }

        //Place Voxels in the OBB Area of the Polygon.
        //float farthest = 0f;
        //int farthestIndex = 0;
        ...
        Voxel[,] voxels = new Voxel[count, count];
        ...
        for (int z = 0; z <= resolution; z++)
        {
            for (int x = 0; x <= resolution; x++, j++)
            {
                ...
                voxels[x, z] = new Voxel(new Vector3(xSpacing, 0, zSpacing), 0.5f);
        ...
        //Create Voxel Squares
        //VoxelSquare[,] squares = new VoxelSquare[resolution, resolution];
        squares = new VoxelSquare[resolution, resolution];
        //Iterate through Voxels and Create Squares that hold the connections to the Voxels and there States.
        for (int z = 0; z <= resolution - 1; z++)
        {
            for (int x = 0; x <= resolution - 1; x++, j++)
            {
                ...
                VoxelSquare square = new VoxelSquare(upper, topRight, right, voxels[x, z]);
                squares[x, z] = square;
            }
        }
        ...
        for (int x = 0; x < squares.GetLength(0); x++)
        {
            ...
            {
                VoxelSquare square = squares[x, z];
                //We are only interseted in Voxels that are not Fully Engulfed in the Shape.
        ...
        //Now we must triangulate the Mesh from the Voxels.
    ...
    static void TriangulateSquare(VoxelSquare square, ref List<int> triangles, ref List<Vector3> vertices)
    {
    ...
    static void MeshFromPoints(ref List<int> triangles, ref List<Vector3> vertices, params Voxel[] points)
    {
    ...
    static void AssignVertices(ref List<Vector3> vertices, params Voxel[] points)
    {
    ...
    static void CreateTriangle(Voxel a, Voxel b, Voxel c, ref List<int> triangles)
    {
    ...
    public static GameObject GenerateGuardRailandPost(Vector3[] line, bool leftFacing, Material railMat, Material postMat) {
        ...
        Mesh railMesh = CiDyUtils.ExtrudeRail(CiDyUtils.ReturnPresetShape(1), line, ourTrans);
    ...
    public static Mesh ExtrudeRail(Vector3[] shape, Vector3[] line, Transform ourTrans)
    {
        ...
        Triangulator tr = new Triangulator(flatShape);
    ...
    public static Mesh ExtrudeRail(Vector3[] leftLine, Vector3[] rightLine, float depth, Transform ourTrans) {
        ...
        Vector2z[] poly = new Vector2z[origPolygon.Length];
        for (int i = 0; i < origPolygon.Length; i++)
        {
            //Project 3D Vectors to a 2D Plane
            poly[i] = new Vector2z(Vector3.Dot(origPolygon[i], u), Vector3.Dot(origPolygon[i], v));
        }
    ...
    public static Mesh CreateRoad(List<Vector3> points, float width, bool isHorizontal)
    {
        ...
        for (int i = 0; i <= n; i++)
        {
            Vector3 thisPoint = points[i];// + left * 0.0f + Vector3.up * 0.0f;
                                          // Except for the last point
            if (i != n)
            {
                // Direction of track
                fwd = points[i + 1] - thisPoint;
                ...
                thisPoint = points[i] + -right * width / 2;//Left
                vs.Add(thisPoint);
                thisPoint = points[i] + right * width / 2;//Right
                vs.Add(thisPoint);
            }
            else
            {
                //No left and right updated just use the last known variables
                thisPoint = points[i] + -right * width / 2;//+ Vector3.up * 0f;
                vs.Add(thisPoint);
                thisPoint = points[i] + right * width / 2;
                vs.Add(thisPoint);
            }
            //Debug.Log("Found LEfT");
        }
        ...
        //Solve Tangent Issue for Bumped Shaders not needed with physics based shading.(Unity5)
        //TangentSolver(mesh);
        //This will probably be moved alot as the User Edits there City
        ;
        return mesh;
    }
    ...
    public static Skeleton CreateStraightSkeleton(List<CiDyVector3> polygon)
    {
        ...
        //Create Skeleton
        Skeleton skeleton = StraightSkeletonNet.SkeletonBuilder.Build(vectors, null, 0);
    ...
    public static List<List<CiDyNode>> CreateSkeletonInset(List<CiDyVector3> polygon, float inset)
    {
        ...
        //Create Skeleton
        Skeleton skeleton = StraightSkeletonNet.SkeletonBuilder.Build(vectors, null, (double)inset);
        Dictionary<Vector2d, double> distance = skeleton.Distances;
        ...
        if (skeleton != null && skeleton.Edges.Count > 0)
        {
            ...
            for (int i = 0; i < skeleton.Edges.Count; i++)
            {
                //Iterate through Polygon of this Edge
                for (int j = 0; j < skeleton.Edges[i].Polygon.Count; j++)
                {
                    //Draw Polygon for Each Edge
                    Vector2d posA = skeleton.Edges[i].Polygon[j];
                    Vector2d posB;
                    if (j == skeleton.Edges[i].Polygon.Count - 1)
                    {
                        //Grab First
                        posB = skeleton.Edges[i].Polygon[0];
                    }
                    else
                    {
                        //Grab Next
                        posB = skeleton.Edges[i].Polygon[j + 1];
                    }
                    ...
                    for (int j = 0; j < skeleton.insetLavs[i].Count; j++)
                    {
                        ...
                        for (int h = 0; h < pointArray.Count; h++)
                        {
                            ...
                            if (origin == chainPos1)
                            {
                                ...
                                if (CiDyUtils.PointInsidePolygon(skeleton.insetLavs[i], new Vector3((float)pointArray[h].endPoint.X, 0, (float)pointArray[h].endPoint.Y)))
                                {
    ...
    public static int CheckLocalConstraints(ref CiDyGraph graph, CiDyEdge newEdge, float testRoadLength, float minRoadLength, float connectPercent, float maxSlope, ref bool corrected, ref CiDyEdge splitEdge)
    {
        ...
        //Contour to terrain
        graph.ContourPathToTerrain(ref rdPath);
