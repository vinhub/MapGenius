...
public class CiDyCell : MonoBehaviour {
	...
	public float lotWidth = 40f;//Cell Building Lots
	public float lotDepth = 40f;//Cell Building Lots
	...
	BuildingType curType = BuildingType.Downtown;
	...
	void GrabAssets(){
		...
		if(clutterObjects.Length == 0){
			...
			clutterObjects = Resources.LoadAll("CiDyResources/ClutterPrefabs", typeof(GameObject)).Cast<GameObject>().ToArray();
		}
		if(pathLight == null){
			pathLight = (GameObject)Resources.Load ("CiDyResources/PathLight", typeof(GameObject));
		}
	...
	public void UpdateCell(){
		...
		//Debug.Log ("Updating Cell OldName("+name+")");
		//cycleNodes = graph.ReturnCell(cellInt, ref filaments);
		/*for(int i = 0;i<cycleNodes.Count;i++){
			Debug.Log(cycleNodes[i].name);
		}*/
		UpdateName ();
    ...
    public void UpdateMesh() {
        ...
        //Debug.Log("AFter First Loop");
        //Show me the Inset Lines
        /*for (int i = 0; i < insetLines.Count; i++) {
            for (int j = 0; j < insetLines[i].Count; j++) {
                CiDyUtils.MarkPoint(insetLines[i][j], j);
            }
        }*/
        ...
        for (int i = 0; i < insetLines.Count; i++)
        {
            ...
            if (maxCurrent == 0) {
                Debug.LogWarning("we are all gonna die");
                break;
            }
        ...
        //Debug.Log("After 4th");
        //Show me the Updated Inset Lines
        /*for (int i = 0; i < insetLines.Count; i++) {
            for (int j = 0; j < insetLines[i].Count; j++) {
                CiDyUtils.MarkPoint(insetLines[i][j], j+999);
            }
        }*/
        ...
        VoxelSquare[,] newVoxels = new VoxelSquare[32, 32];
        ...
        if (createSideWalks)
        {
            ...
            List<List<CiDyNode>> insetNodes = CiDyUtils.InsetPolygon(insetPoly, 0.01f);
            lotPoly = insetNodes;
					...
					for(int k = 0;k<stepSpace;k++){
						...
						if(pathLight){
							if(lightsCurDist >= pathLightSpacing){
								...
								if(!cantPlace){
									GameObject newLight = (GameObject)Instantiate(pathLight,lightPoint,Quaternion.identity);
						...
						if(clutterObjects.Length > 0){
							...
							if(cluttersCurDist >= pathClutterSpacing){
								...
								if(!cantPlace){
									...
									GameObject newClutter = (GameObject)Instantiate(clutterObjects[objPlace],clutterPoint,Quaternion.identity);
	...
	void ExtrudeLots(){
		if(curType != buildingType){
			//Clear Prefabs so we Know that the User Has Changed the BuildingType
			prefabBuildings = new GameObject[0];
        ...
        if(usePrefabBuildings){
			//Place Prefab Buildings
			if(prefabBuildings.Length == 0 && autoFillBuildings){
				//Auto Fill from Resources
				switch(buildingType){
				case BuildingType.Downtown:
					prefabBuildings = Resources.LoadAll("CiDyResources/DownTownPrefabs", typeof(GameObject)).Cast<GameObject>().ToArray();
					break;
				case BuildingType.Industrial:
					prefabBuildings = Resources.LoadAll("CiDyResources/IndustrialPrefabs", typeof(GameObject)).Cast<GameObject>().ToArray();
					break;
				case BuildingType.Residential:
					prefabBuildings = Resources.LoadAll("CiDyResources/ResidentialPrefabs", typeof(GameObject)).Cast<GameObject>().ToArray();
					break;
				}
			}
            ...
            {
                lotBounds[n] = interiorPoints[n] + this.transform.position;
            }
            //For Debug
            ...
            for (int i = 0;i<cidyLots.Count;i++){
                ...
                {
                    //Prefab Bounds
                    Bounds prefabBounds = prefabBuildings[c].GetComponentInChildren<Renderer>().bounds;
                ...
                if (!huddleBuildings)
                {
                    ...
                    while (!placed)
                    {
                        ...
                        GameObject placedBuilding = (GameObject)Instantiate(prefabBuildings[pickedPrefab], Vector3.zero, Quaternion.identity);
                        //Prefab Bounds
                        Bounds prefabBounds = placedBuilding.GetComponentInChildren<Renderer>().bounds;
                        Vector3 extents = prefabBounds.extents;
                        Vector3 boundsCntr = prefabBounds.center;
                        ...
                        if (!CiDyUtils.BoundsIntersect(lotBoundary, boundFootPrint))
                        {
                            ...
                            else
                            {
                                ...
                                //RaycastHit hit;
                                //Shoot Cast Downward
                                /*if (Physics.Raycast(placedBuilding.transform.position + (Vector3.up * 100), Vector3.down, out hit, Mathf.Infinity, graph.roadMask))
                                {
                                    //Project it to the Y Access
                                    placedBuilding.transform.position = hit.point + (Vector3.up * sideWalkHeight);//new Vector3(placedBuilding.transform.position.x, hit.point.y, placedBuilding.transform.position.z);
                                }
                                else
                                {*/
                                    //Project it to the Y Access
                    ...
                    if (bestFit != -1)
                    {
                        ...
                        GameObject newBuilding = (GameObject)Instantiate(prefabBuildings[bestFit], Vector3.zero, Quaternion.identity);
                        //Prefab Bounds
                        Bounds prefabBounds = newBuilding.GetComponentInChildren<Renderer>().bounds;
                        Vector3 extents = prefabBounds.extents;
                        Vector3 boundsCntr = prefabBounds.center;
                        //Move 
                        ...
                        //RaycastHit hit;
                        //Shoot Cast Downward
                        /*if (Physics.Raycast(newBuilding.transform.position + (Vector3.up * 100), Vector3.down, out hit, Mathf.Infinity, graph.roadMask))
                        {
                            //Project it to the Y Access
                            newBuilding.transform.position = hit.point + (Vector3.up * sideWalkHeight);//new Vector3(placedBuilding.transform.position.x, hit.point.y, placedBuilding.transform.position.z);
                        }
                        else
                        {*/
                            //Project it to the Y Access
                    ...
                    for (int t = 0; t < cidyLots[i].roadSides.Count; t++)
                    {
                        ...
                        while (remainingRoadAccess > 0)
                        {
                            ...
                            if (remainingRoadAccess >= smallestWidth)
                            {
                                ...
                                for (int n = 0; n < prefabWidths.Count; n++) {
                                    if (prefabWidths[n] == smallestWidth) {
                                        pickedPrefab = n;
                                        break;
                                    }
                                }*/
                            ...
                            int leftFntBoundPnt = 1;//Default reference to first point in Bounds
                            int rightFntBoundPnt = 0;//Default reference to Second Point in Bounds
                            /*int leftBckBoundPnt = 2;//Default reference to Third Point in Bounds
                            //Iterate through Bound
                            //Area = L*W;//Calculate Area of Bounding Box
                            float width = Vector3.Distance(boundFootPrint[leftFntBoundPnt], boundFootPrint[rightFntBoundPnt]);//Width
                            float length = Vector3.Distance(boundFootPrint[leftFntBoundPnt], boundFootPrint[leftBckBoundPnt]);//Length
                            float area = length * width;//Area*/
                            ...
                            if (prefabWidths[pickedPrefab] <= remainingRoadAccess && !CiDyUtils.BoundsIntersect(lotBoundary, boundFootPrint))
                            {
                                ...
                                if (!CiDyUtils.BoundsIntersect(boundFootPrint, buildings.ToArray()))
                                {
                                    ...
                                    //RaycastHit hit;
                                    //Shoot Cast Downward
                                    /*if (Physics.Raycast(placedBuilding.transform.position + (Vector3.up * 1000), Vector3.down, out hit, Mathf.Infinity, graph.roadMask))
                                    {
                                        //Project it to the Y Access
                                        placedBuilding.transform.position = hit.point + (Vector3.up * sideWalkHeight);//new Vector3(placedBuilding.transform.position.x, hit.point.y, placedBuilding.transform.position.z);
                            ...
                            else
                            {
                                ...
                                if (smallestWidth >= remainingRoadAccess)
                                {
                                    ...
                                    /*if (remainingRoadAccess > 0)
                                    {
                                        float moveDist = remainingRoadAccess / 2;
                                        for (int r = 0; r < rowBuildings.Count; r++)
                                        {
                                            rowBuildings[r].transform.position += (-right * moveDist);
                                        }
                                    }*/
                                    //Clear remaining 
    ...
    private List<Vector3> tmpPoints = new List<Vector3>(0);
    ...
    void CreateSideWalks()
    {
        ...
        //insetTmpPoints = new List<List<Vector3>>(0);
        tmpPoints = new List<Vector3>(0);
        for (int i = 0; i < insetPoly.Count; i++)
        {
            //GameObject sphere = CiDyUtils.MarkPoint(insetPoly[i].pos, i);
            //sphere.GetComponent<MeshRenderer>().material.color = Color.red;
            if (i == 0)
            {
                insetPoly[i].position = 0;
                //CiDyUtils.MarkPoint(insetPoly[i].pos,100);
            }
            else if (i == insetPoly.Count - 1)
            {
                insetPoly[i].position = 1;
                //CiDyUtils.MarkPoint(insetPoly[i].pos, 999);
            }
            else {
                insetPoly[i].position = 2;
            }
            tmpPoints.Add(insetPoly[i].pos);
        }
        ...
        for (int i = 0; i < insetLines.Count; i++)
        {
            ...
            {
                //CiDyUtils.MarkPoint(insetCurves[i][j], j);
                hole.Add(insetCurves[i][j]);
            }
        }

        //CiDyUtils.ExtrudeSideWalk(interiorPoints, hole, sideWalkHeight, transform);
        ...
        List<List<CiDyNode>> insetNodes = CiDyUtils.CreateSkeletonInset(insetPoly, sideWalkWidth);
